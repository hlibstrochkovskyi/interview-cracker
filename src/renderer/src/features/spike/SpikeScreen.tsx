    ...
    <div className="flex h-screen flex-col bg-bg">
      ...
      <main className="grid flex-1 grid-cols-1 gap-6 overflow-auto px-8 py-6 lg:grid-cols-[1.4fr_1fr]">
        <section className="flex flex-col items-center justify-center gap-8">
          ...
          <Card className="min-h-28 w-full max-w-xl">
            <p className="text-xs uppercase tracking-wide text-text-muted">Interviewer</p>
        ...
        <section className="flex flex-col gap-4">
          ...
          <Card className="flex-1 overflow-auto">
            <h3 className="text-sm font-medium text-text-muted">Turns</h3>
            <ul className="mt-3 space-y-2 text-sm">
              {turns.length === 0 && <li className="text-text-muted">No turns yet…</li>}
              {turns.map((t) => (
                <li key={t.turn} className="flex items-center justify-between gap-3">
                  <span className="truncate text-text-muted">#{t.turn}</span>
                  <span className="truncate">{t.assistantText}</span>
